...
let fake_encoder = Pbrt.Encoder.create ()
let header_buffer = Bytes.create 5

let () =
  H2_load_test_rpc.encode_pb_packet
    { timestamp = Unix.gettimeofday (); audio = Bytes.make 160 (Char.chr 0xD5) }
    fake_encoder

let msg = Pbrt.Encoder.to_bytes fake_encoder
let () = Grpc.Message.fill_header ~length:(Bytes.length msg) header_buffer
    ...
    *)
    ...
    ( {
        Grpc_client_eio.Io.write =
          (fun (* let header_buffer = Bytes.create 5 in *)
                 _input
          ->
